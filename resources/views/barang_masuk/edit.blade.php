...
@section('content')
    <div class="card card-outline card-primary">
        ...
        <div class="card-body">
            ...
            @else
                <form method="POST" action="{{ url('/barang_masuk/' . $barang_masuk->id_transaksi) }}" class="form-horizontal">
                    @csrf
                    {!! method_field('PUT') !!}
                        ...
                        <div class="col-sm-10">
                            <select class="form-control" id="barang" name="id_barang" required>
                                ...
                                @foreach ($barang as $item)
                                    <option value="{{ $item->id_barang }}" @if ($item->id_barang == $barang_masuk->id_barang) selected @endif>
                    ...
                    <input type="hidden" value="{{$barang_masuk->id_barang}}" name="id_barang">
                    <div class="form-group row">
                        <label for="nama_barang" class="col-sm-2 col-form-label">Jumlah Masuk</label>
                        <div class="col-sm-10">
                            <input type="text" name="barang_masuk"
                                class="form-control @error('barang_masuk')
                              is-invalid
                          @enderror"
                                id="barang_masuk" placeholder="Jumlah Masuk"
                                value="{{ old('barang_masuk', $barang_masuk->barang_masuk) }}">
                            @error('barang_masuk')
                    ...
                    <div class="form-group row">
                      ...
                      @enderror"
                                ...
                                value="{{ old('tgl_transaksi', date('Y-m-d', strtotime($barang_masuk->tgl_transaksi))) }}">
                            @error('tgl_transaksi')
                                <div class="invalid-feedback" role="alert">
                                    {{ $message }}
                                </div>
                    ...
                    <div class="form-group row">
                        ...
                        <div class="col-sm-10">
                          ...
                          is-invalid
                      @enderror"
                                id="status" placeholder="Stok" value="{{ old('status', $barang_masuk->status) }}">
                            @error('status')
                                <div class="invalid-feedback" role="alert">
                                    {{ $message }}
                                </div>
